...
public class EventsDAO {
    ...
    private static final Object HASH_KEY = "Events";
...
//            restTemplate.postForObject("http://localhost:8080/producer", payload, String.class);
    ...
    public JsonNode findById(String id) {
        List<JsonNode> events = template.opsForHash().values(HASH_KEY);
        for (JsonNode e : events) {
            if (e.get("id").asText().equals(id)) {
                return e;
            }
        }
        return null;
    ...
    public String getSchema() {
        return template.opsForHash().values("EventSchema").toString();
    }
