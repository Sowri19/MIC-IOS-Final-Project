...
@Repository
public class ComediansDAO {

    private static final Object HASH_KEY = "Comedians";
...
//            restTemplate.postForObject("http://localhost:8080/producer", payload, String.class);
    ...
    public JsonNode findById(String id) {
        List<JsonNode> comedians = template.opsForHash().values(HASH_KEY);
        for (JsonNode c : comedians) {
    ...
    public String deleteRate(String id) {
        ...
        return "Comedian Removed!";
    }

    public String getSchema() {
        return template.opsForHash().values("CSchema").toString();
    }
